...
<body>	
	<table class="listTable">
		...
		<tr>			
			<td colspan="2" class="boardListLink"><a href="boardList.jsp">리스트</a></td>
			<td colspan="3" class="listWrite"><a href="writeForm.jsp">글쓰기</a></td>
		</tr>		
		</c:if>
		<!-- 검색 요청이 아닌 일반 게시 글 리스트 요청인 경우 아래를 화면에 표시 한다. -->
	...
	<c:if test="${ searchOption and not empty bList }">
		<!-- 
			EL를 이용해 링크 설정에 사용하기 위해 pageContext 영역의 속성에 저장 
		-->
		...
		<tr class="listTr">
			...
			<td class="listTdTitle">
				<a href="boardDetail.jsp?no=${ b.no }&pageNum=${currentPage}
			...
			<td colspan="5" class="listPage">
				 ...
				 * startPage로 이동할 수 있다.
			 	...
			 	<c:if test="${ startPage > pageGroup }">
					<a href="boardList.jsp?pageNum=${ startPage - pageGroup }
				...
				<c:forEach var="i" begin="${ startPage }" end="${ endPage }">
					...
					<c:if test="${ i != currentPage }">
						<a href="boardList.jsp?pageNum=${ i }&type=${ type }
							&keyword=${ keyword }">[ ${ i } ]</a>
					</c:if>			
				</c:forEach>
				<%-- 
