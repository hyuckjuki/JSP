...
<% 
	// 게시 글의 수가 0보다 크면 즉 게시 글이 하나라도 존재한다면
	if(listCount > 0) {
		...
		/* 현재 페이지 그룹의 시작 페이지가 PAGE_GROUP보다 크다는 것은 이전 페이지
		 * 그룹이 존재한다는 것으로 현재 페이지 그룹의 시작 페이지에 PAGE_GROUP를 
		 * 마이너스하여 링크를 설정하면 이전 페이지 그룹의 startPage로 이동할 수 있다.
		 **/
		if(startPage > PAGE_GROUP) {	%>		
			<a href="boardList.jsp?pageNum=
				<%= startPage - PAGE_GROUP %>">[이전]</a>		
<% 	}
...
<%		}
		 ...
		 * 그룹이 존재한다는 것으로 현재 페이지 그룹의 시작 페이지에 PAGE_GROUP을
		 * 플러스하여 링크를 설정하면 다음 페이지 그룹의 startPage로 이동할 수 있다.
		 **/
		if(endPage < pageCount) {	%>		
			<a href="boardList.jsp?pageNum=
				<%= startPage + PAGE_GROUP %>">[다음]</a>		
<%	}
	}
%>			
		</td>
